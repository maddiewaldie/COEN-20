...
void __attribute__((weak)) UseLDRB(void *dst, void *src)
    {
    int byte ;
...
void __attribute__((weak)) UseLDRH(void *dst, void *src)
    {
    int halfword ;

    for (halfword = 0; halfword < 512/2; halfword++)
        {
        ((uint16_t *) dst)[halfword] = ((uint16_t *) src)[halfword] ;
        }
    }

void __attribute__((weak)) UseLDR(void *dst, void *src)
    {
    int word ;

    for (word = 0; word < 512/4; word++)
        {
        ((uint32_t *) dst)[word] = ((uint32_t *) src)[word] ;
        }
    }

void __attribute__((weak)) UseLDRD(void *dst, void *src)
    {
    int doubleword ;

    for (doubleword = 0; doubleword < 512/8; doubleword++)
        {
        ((uint64_t *) dst)[doubleword] = ((uint64_t *) src)[doubleword] ;
        }
    }

void __attribute__((weak)) UseLDM(void *dst, void *src)
    {
    uint32_t regs[8] ;
    int set, reg ;

    for (set = 0; set < (512/4)/8; set++)
        {
        // Simulate LDMIA loading 8 registers
        for (reg = 0; reg < 8; reg++)
            {
            regs[reg] = ((uint32_t *) src)[8*set + reg] ;
            }

        // Sumulate STMIA storing 8 registers
        for (reg = 0; reg < 8; reg++)
            {
            ((uint32_t *) dst)[8*set + reg] = regs[reg] ;
            }
        }
    }
...
int main(void)
    ...
    InitializeHardware(HEADER, "Lab 4A: Copying Data Quickly") ;
    LEDs(0, 1) ;
